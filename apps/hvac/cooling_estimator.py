...
class CoolingEstimator:
    _COOLING_TARGET_OFFSET = Celsius(-2.0)
    ...
    def estimate_temperature(self, state: HvacState, now: datetime) -> Celsius | None:
        ...
        if state.is_eco_mode:
            temperature_target = self.configuration.cooling_temp_eco_on
            ...
            temperature_target = self.configuration.cooling_temp_eco_off
        ...
        if in_boost_window:
            temperature_target += self._COOLING_TARGET_OFFSET
        ...
        if temperature_target != state.cooling_target_temperature:
            self.appdaemon_logger.log(
                "Cooling temperature target: %s, adjustment: %s",
                temperature_target,
